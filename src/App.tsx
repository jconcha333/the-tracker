...
export default function App() {
  ...
  async function fetchDayHistory() {
    ...
    try {
      ...
      const { data: noteData } = await theTracker.from('workout_notes').select('*').eq('note_date', selectedDate).order('created_at', { ascending: false });
          ...
          <div style={{...styles.headerContent, maxWidth: activeTab === 'STATS' ? '100%' : '400px'}}>
              <header style={styles.headerArea}>
                ...
                <div style={styles.topNavRowCentered}>
                  ...
                  <button onClick={() => setSelectedDate(new Date().toISOString().split('T')[0])} style={{...styles.todayBtn, color: selectedDate === new Date().toISOString().split('T')[0] ? '#FFF' : '#444'}}>TODAY</button>
                </div>
              </header>
          </div>
      </div>
          ...
          {activeTab === 'NOTES' && (
            <div style={styles.notesPanel}>
              ...
              <div style={styles.notesList}>
                {notesList.map(note => (
                  <div key={note.id} style={styles.noteCard}>
                    <div style={styles.noteHeader}>
                      <span style={styles.noteTime}>{new Date(note.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                ...
                {notesList.length === 0 && <div style={styles.emptyStats}>NO NOTES FOR THIS DAY</div>}
              </div>
            </div>
          )}
          ...
          {activeTab === 'TRACKER' && (
            <>
              ...
              <div style={styles.timerBar}>
                <span className={timer === 0 ? 'timer-done' : ''} style={{...styles.timerText, cursor: timer !== null ? 'pointer' : 'default'}} onClick={() => setTimer(null)}>{timer !== null ? (timer === 0 ? "REST DONE" : `CANCEL: ${timer}S`) : <Clock size={14}/>}</span>
                <div style={styles.timerCircles}>{[30, 45, 60, 120, 180].map(s => <button key={s} onClick={() => { if (audioCtx.current?.state === 'suspended') audioCtx.current.resume(); setTimer(s); }} style={styles.timerCircle}>{s}</button>)}</div>
              </div>
                              ...
                              <div key={set.id} style={styles.setRow}>
                                  <div style={styles.setMain} onClick={() => toggleComplete(set.id, set.is_completed)}>
                                    ...
                                    <span style={{...styles.setText, textDecoration: set.is_completed ? 'line-through' : 'none', opacity: set.is_completed ? 0.3 : 1}}>
                                        S{i+1} — {set.category === 'STRENGTH' ? `${set.weight_lbs}LB x ` : ''}{set.reps} {set.category === 'CARDIO' ? 'MIN' : (set.category === 'STRENGTH' ? 'REPS' : 'SEC')}
                                    </span>
                                  </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  headerContent: { width: '100%', padding: 'env(safe-area-inset-top, 20px) 20px 8px 20px', boxSizing: 'border-box' },
  scrollArea: { flex: 1, overflowY: 'auto', paddingTop: '80px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  ...
  headerTitleGroup: { width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' },
  ...
  miniProgressContainer: { width: '100%', height: '4px', backgroundColor: '#050505', border: '1px solid #1A1A1A', position: 'relative', overflow: 'hidden', marginBottom: '6px' },
  miniProgressBar: { height: '100%', backgroundColor: '#FFF', transition: 'width 0.4s ease' },
  topNavRowCentered: { display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', gap: '20px', paddingBottom: '4px' },
  ...
  form: { display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px', marginBottom: '10px' },
  ...
  button: { backgroundColor: '#FFF', color: '#000', border: 'none', padding: '16px', fontWeight: 600, fontSize: '11px', letterSpacing: '2px', cursor: 'pointer', marginBottom: '2px', width: '100%' },
  timerBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderTop: '1px solid #1A1A1A', borderBottom: '1px solid #1A1A1A', marginBottom: '15px' },
  timerText: { fontSize: '10px', color: '#444' },
  timerCircles: { display: 'flex', gap: '6px' },
  timerCircle: { background: 'none', border: '1px solid #1A1A1A', color: '#FFF', width: '32px', height: '32px', borderRadius: '50%', fontSize: '10px', cursor: 'pointer' },
  ...
  setRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: '12px' },
